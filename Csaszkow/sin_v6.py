...
if __name__ == '__main__':
    ...
    def learn(network,samples, epochs=500000, lrate=.2, momentum=0.1):
        # Train 
    ...
    network = MLP(1,20,20,1)
    samples = np.zeros(100, dtype=[('x',  float, 1), ('y',  float, 1)])
    samples['x'] = np.append(np.linspace(-2*np.pi, -1*np.pi, 50),np.linspace(np.pi, 2*np.pi, 50))
    samples['y'] = np.sin(samples['x']) 
    
    teszt = np.zeros(50, dtype=[('x',  float, 1),  ('y',  float, 1)])
    teszt['x'] = np.linspace(-1*np.pi,1*np.pi,50)
