...
if __name__ == '__main__':
    ...
    samples = np.zeros(60, dtype=[('x',  float, 1), ('y',  float, 1)])
    samples['x'] = np.append(np.linspace(-2*np.pi, -1*np.pi, 30),np.linspace(np.pi, 2*np.pi, 30))
    samples['y'] = np.sin(samples['x']) 
    
    teszt = np.zeros(60, dtype=[('x',  float, 1)])
    teszt['x'] = np.linspace(-2*np.pi,2*np.pi,60)
