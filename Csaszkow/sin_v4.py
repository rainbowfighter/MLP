...
if __name__ == '__main__':
    ...
    samples = np.zeros(30, dtype=[('x',  float, 1), ('y',  float, 1)])
    samples['x'] = np.linspace(0,2*np.pi,30)
    samples['y'] = np.sin(samples['x']) 
    
    teszt = np.zeros(30, dtype=[('x',  float, 1)])
    teszt['x'] = np.linspace(2*np.pi,3*np.pi,30)
